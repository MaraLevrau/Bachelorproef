...
Under the assumption that all claims are independent of each other (in the probability theoretical sense), the calculation of these risk measures for $S$ is very easy. However, in practice one cannot make this assumption: for instance, natural disasters can wreak havoc in a large residential area, resulting in several claims relating to housing damage all at once.


\subsection{Expected utility theory} 
The classical framework used to reason about the rationality of making decisions was first proposed in the fourties by John von Neumann and Oskar Morgenstern \cite{theoryOfGamesAndEconomicBehavior}. Central in their reasoning is the concept of a \textit{utility function} $u: \mathbb{R} \to \mathbb{R}$, which quantifies the degree to which the agent under consideration values accruing a given amount of capital. Von Neumann and Morgenstern posit that any ``rational'' agent has an intrinsic utility function which they use to determine a reasonable course of action: the rational choice is precisely the choice whose outcome is expected to maximise the value of the agent's utility function. If a utility function is concave (i.e. defined on a compact set $I$ of $\mathbb{R}$ such that $\forall x, y \in I, \forall t \in [0, 1]: u(tx + (1-t)y) \geq t u(x) + (1-t) u(y)$), the corresponding agent is called \textit{risk averse}.



\section{Assumptions}\label{section: assumptions}
In this report we will assume that all random variables are defined on a common probability space $(\mathbb{R}, \mathcal{B}(\mathbb{R}),P)$. Additionally we assume that the expectation $\mathbb{E}[X]$ exists and is finite for every random variable.

We will focus on the case where all the risks follow a lognormal distribution. In particular this means that the random variables that interest us have continuous and strictly increasing cumulative distribution functions. Throughout this report we will only prove the relevant results for continuous random variables. In addition we will assume that all cumulative distribution functions $F$ have an interval $I = (a,b)\subseteq \mathbb{R}$ (where $a$ and $b$ are possibly infinite) such that the following holds:
...
This implies that $F:I\rightarrow F(I)$ is a bijection and hence invertible. As a result of this assumption, we can uniquely define $F^{-1}(0) := a$ and $F^{-1}(1) := b$ (where $a$ and $b$ are possibly infinite) for every random variable with cumulative distribution function $F$. Thus $$F:[a,b]\subseteq \mathbb{R}\cup\{\pm\infty\}\rightarrow[0,1]$$ is a uniquely defined, continuous and strictly increasing bijection. 
...
Intuitively, one can think of the Value-at-Risk as the quantity that answers the question ``What loss is exceeded with probability $1 - p$?'', whereas the Tail Value-at-Risk answers the question ``How bad is the situation once we indeed exceed that $1-p$ loss?'' This highlights one of the most often criticised weaknesses of the VaR: in a sense, it is not sensitive to tail behaviour. Two risks could have an identical probability of exceeding a certain loss, but one could have a much thicker tail, indicating that the latter is more disastrous in case of the loss being exceeded. 

\section{Copulas}
A copula is a multivariate cumulative distribution function for a random vector with uniform marginals which can be used to model the dependence structure between random variables. An $n$-dimensional copula is defined as follows.
\\
...
\begin{theorem}[\textbf{Sklar's theorem}, part 1]\cite[Theorem 2]{Copulas}\label{Sklar's theorem}
    ...
    $$C: [0,1]^n\rightarrow [0,1]: (u_1,u_2,\dots,u_n)\mapsto  F_{\underline{X}}(F_1^{-1}(u_1), F_2^{-1}(u_2),\dots,F_n^{-1}(u_n)).$$ 
\end{theorem}
\begin{proof}
    We have
    \begin{align*}
    ...
    \end{align*}
    for all $(x_1,x_2,\dots,x_n)\in[a_1,b_1]\times[a_2,b_2]\times \dots\times [a_n,b_n]$. If we define 
    $$C(u_1,u_2,\dots,u_n) := F_{\underline{X}}(F_1^{-1}(u_1), F_2^{-1}(u_2),\dots,F_n^{-1}(u_n))$$ 
    for $(u_1,u_2\dots,u_n)\in [0,1]^n$, we see that 
    $$F_{\underline{X}}(x_1,x_2,\dots,x_n) = C(F_1(x_1),F_2(x_2),\dots,F_n(x_n)).$$
    ...
    \end{align*}
    for $(u_1,u_2,\dots,u_n)\in [0,1]^n$. Thus $C$ is a joint cumulative distribution function with uniform marginals and thus indeed a copula.\\\\
    ...
    We need to prove that $C(u_1,\dots,u_n) = D(u_1,\dots,u_n)$ for all $(u_1, \dots,u_n)\in [0,1]^n$. $F_i:[a_i,b_i]\rightarrow [0,1]$ is a bijection for every $i\in \{1,\dots,n\}$. Suppose $(u_1,\dots,u_n)\in [0,1]^n$. For every $u_i$ there exists an $x_i\in [a_i,b_i]$ such that $F_i(x_i) = u_i$. This means that
    \begin{align*}
...
\end{theorem}
\begin{proof}
    Assume that $C$ is a copula and $F_1,F_2,\dots,F_n$ univariate cumulative distribution functions. Define $F_{\underline{X}}(x_1,x_2,\dots,x_n) := C(F_1(x_1),F_2(x_2),\dots,F_n(x_n))$ for $(x_1,\dots,x_n)\in [a_1,b_1]\times \dots \times [a_n,b_n]$ . Since 
    ...
    $F_{\underline{X}}(x_1,x_2,\dots,x_n)$ is a joint cumulative distribution function. We claim that this also implies that $F_{\underline{X}}$ has marginals $F_1,F_2,\dots,F_n$. Indeed, for $j\in \{1,2,\dots,n\}$ and $x_j\in [a_j,b_j]$, we have
    \begin{align*}
...
Using this theorem, we see that given a random vector $(X_1,X_2,\dots,X_n)$ with marginal distribution functions $F_1,F_2,\dots,F_n$, a copula $C$ combines (i.e. `couples')  the marginal distributions to form a joint cumulative distribution function. Thus a copula captures the dependence structure of a random vector. If the dependence structure of a random vector is unknown, we can try to estimate a copula based on the data. Theorem \ref{Sklar's theorem} guarantees uniqueness of the copula, but only under the assumption of perfect knowledge. When only partial knowledge is available, multiple copulas may satisfy the given constraints, this is the case if only the marginal distributions and the correlation matrix are provided. To explore the impact of the choice of copula, it is useful to first look at a few commonly used copulas.
\\
...
\begin{remark}\label{remark: inverse cdf property}
    Equation \eqref{eq: inverse cdf property} also holds for $p\in \{0,1\}$ by taking the limits since both sides are continuous functions of $p$, but in that case the values may become infinite.
\end{remark}
...
\begin{proposition}\cite[Corollary 1]{kaas-2000}\label{property stop-loss premium}
    Suppose $U\sim \text{Uniform}(0,1)$ a random variable, $(X_1,X_2,\dots,X_n)$ a random vector with continuous marginal cdfs $F_1,F_2,\dots,F_n$ and $S^u = F_1^{-1}(U) + F_2^{-1}(U) + \dots + F_n^{-1}(U)$. If  $d\in (F_{S^u}^{-1}(0), F_{S^u}^{-1}(1))$ then
...
\begin{proof} 
    ...
    \end{equation}
    for every  $d\in (F_{S^u}^{-1}(0), F_{S^u}^{-1}(1))$. Suppose $a = F_{S^u}^{-1}(0)$ and $b = F_{S^u}^{-1}(1)$ ($a$ and $b$ may be infinity), then  $F_{S^u}: (a,b) \rightarrow (0,1)$ is a bijection. We are left to prove that the inequality \eqref{eq: stop-loss premiums inequality} holds for $d\in \mathbb{R}\setminus(a,b)$.\\
...
\begin{align*}
&\text{VaR}_p[S^u] = F^{-1}_{S^u}(p) = \sum_{i=1}^nF_i^{-1}(p) = \sum_{i=1}^n\text{VaR}_p[X_i]\\
&\text{TVaR}_p[S^u] = \frac{1}{1-p}\int_p^1F^{-1}_{S^u}(q)\;dq = \frac{1}{1-p}\sum_{i=1}^n\left(\int_p^1F_i^{-1}(q)\;dq\right) = \sum_{i=1}^n\text{TVaR}_p[X_i].
\end{align*}
...
\end{equation}
Then we can write $S^l = f(U)$ for $U\sim \text{Uniform}(0,1)$. We have not yet made any assumptions about $\beta_1,\beta_2,\dots,\beta_n$, which limits what we can say about the general behaviour of $f$. If $f$ is strictly increasing, the Value-at-Risk of $S^l$ can be determined in closed-form. The Tail Value-at-Risk and stop-loss premiums can also be expressed in terms of the inverse, though these expressions will generally still involve an integration. Furthermore, since $f$ is strictly increasing, the TVaR and the stop-loss premiums of $S^l$ can be written as the sum of the TVaR and stop-loss premiums of the marginal distributions. Since the TVaR and stop-loss premiums of a lognormally distributed random variable are available in closed-form, it follows that those of $S^l$ can also be determined in closed form. Notice that $f$ is strictly increasing if we assume that $\rho_i > 0$ for every $i$. Since
\begin{align*}
...
This expression seems complicated but we only need to calculate it once per sample. 
...
The procedures outlined above can be compared on two fronts, both of practical concern: how accurate are the methods, and how quickly can we calculate the results? In both cases, the closed-form for the lower bound is a very satisfactory estimator: for adequately small variances of the $X_i$'s, we can keep the relative error of this approximation within a percent (see also figure \ref{fig: sigmaVgl}), and the computation time is approximately linear in the number of underwritten premiums, with a value of just under 200 ms for $n = 50$. 
